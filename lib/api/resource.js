 ...
 */
async function refreshPublicPages(res, paths) {
  await revalidatePublicPages(res, paths);
}
...
export function defineResource(config) {
  ...
  /**
   * Which public pages this mutation invalidated.
   *
   * Most entities appear only on the home page and the default list is right for
   * them. A blog post also has a page of its own whose path depends on its slug —
   * and the slug can change in the same save — so the entity supplies a function
   * rather than the resource layer growing a special case for one model.
   *
   * Falling back to the default on an empty result is deliberate: a hook that
   * returns nothing has probably received a shape it did not expect, and skipping
   * revalidation entirely would be a silently stale site.
   */
  function pathsFor({ record, existing }) {
    if (!revalidatePaths) return undefined;

    const paths = revalidatePaths({ record, existing });
  ...
  async function create(req, res) {
    ...
    await refreshPublicPages(res, pathsFor({ record: created, existing: null }));
  ...
  async function update(req, res) {
    ...
    // Captured from inside the transaction because the *previous* row is what
    // says which paths were valid before this save — a renamed slug leaves its old
    // URL serving old content otherwise.
    let before = null;
    ...
    await refreshPublicPages(res, pathsFor({ record: updated, existing: before }));
  ...
  async function remove(req, res) {
    ...
    await refreshPublicPages(res, pathsFor({ record: deleted, existing: deleted }));
  ...
  async function reorder(req, res) {
    ...
    await prisma.$transaction(async (tx) => {
      // Sequential, not Promise.all: concurrent queries inside one interactive
      // transaction share a connection and fail over the Neon pooler with P2028.
      for (const [index, id] of ids.entries()) {
        await tx[delegate].update({ where: { id }, data: { order: index } });
      }
