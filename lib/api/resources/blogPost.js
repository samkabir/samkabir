...
import { postPaths } from '../../revalidate.js';
...
export const blogPostResource = defineResource({
  ...
  /**
   * A post owns a page of its own, so the default `['/', '/blog']` is not enough.
   *
   * Both slugs are passed because a rename is a normal edit: the new URL has to
   * start working and the old one has to stop serving the old content. Missing
   * the old one is the quiet failure — the post looks un-edited at the address
   * someone already shared, and nothing anywhere reports a problem.
   *
   * On a delete, `record` and `existing` are the same row, so this resolves to
   * that single slug and rebuilds its page into a 404.
   */
  revalidatePaths: ({ record, existing }) =>
    postPaths({ slug: record?.slug, previousSlug: existing?.slug }),
});
