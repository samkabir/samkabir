...
export const accountHandler = createHandler({
   ...
   * field allowlist, plus whether a password is set. The dashboard needs that
   * last flag to decide between "change password" and "set a password" for an
   * account that has only ever used Google — and it is a boolean, not the hash.
   */
  GET: async (req, res) => {
    const { passwordHash } = await prisma.adminUser.findUniqueOrThrow({
      where: { id: req.adminUser.id },
      select: { passwordHash: true },
    });

    res.status(200).json({
      item: { ...req.adminUser, hasPassword: Boolean(passwordHash) },
    });
  },
});
