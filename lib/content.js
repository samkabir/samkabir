...
export async function getPostNeighbours(publishedAt, client = prisma) {
  ...
  /**
   * `livePosts()` and the neighbour bound both constrain `publishedAt`, so they
   * are *merged* rather than spread — `{ ...livePosts(), publishedAt: { lt: at } }`
   * would let the second `publishedAt` overwrite the first, dropping the
   * `lte: now` cap and offering a scheduled future post as a neighbour link,
   * slug and title, before its publication date. Merging keeps `not: null` and
   * `lte: now` alongside the directional bound.
   */
  const live = livePosts();

  const [previous, next] = await Promise.all([
    client.blogPost.findFirst({
      where: { ...live, publishedAt: { ...live.publishedAt, lt: at } },
      orderBy: { publishedAt: 'desc' },
      select,
    }),
    client.blogPost.findFirst({
      where: { ...live, publishedAt: { ...live.publishedAt, gt: at } },
...
/**
 * Everything the home page needs, fired concurrently.
 *
 * These are all reads, so there is no atomicity to preserve. They run through a
 * plain `Promise.all` rather than an interactive `$transaction`: concurrent
 * queries inside a single interactive transaction share one connection and break
 * over Neon's pooled connection with P2028 ("Transaction not found") — which is
 * how a static build against the production pooler fails. `Promise.all` still
 * overlaps the queries, and Neon wakes its suspended compute only once per
 * endpoint regardless, so the "don't pay the wake-up cost nine times in sequence"
 * intent is kept without the transaction.
 *
...
export async function getPageContent() {
  const [
    ...
    getProfile(prisma),
    getSeoSettings(prisma),
    getSectionCopy(prisma),
    getSkills(prisma),
    getEducation(prisma),
    getExperiences(prisma),
    getContractualExperiences(prisma),
    getProjects(prisma),
    getSocialLinks(prisma),
    hasActiveResume(prisma),
    hasPublishedPosts(prisma),
  ]);
