/**
 * Display formatting, shared by the dashboard and the public site.
 *
 ...
 * only ever decoration. Fixed formats avoid the whole class of problem, and the
 * dashboard has exactly one reader, whose date format is known.
 *
 * Phase 7 made `formatTimeline` and `formatYearRange` load-bearing for the public
 * site too: `lib/content.js` calls them so a role's dates become the string a
 * visitor reads. Sharing them rather than writing a second formatter is the
 * point — the dashboard is where these dates are edited, so a site that spelled
 * them differently would make every edit a guess.
 */
